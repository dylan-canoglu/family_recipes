        ...
        <div className="p-4 border-t border-slate-100">
          {user ? (
            <div className={`flex flex-col ${!isSidebarOpen && 'items-center'}`}>
              {isSidebarOpen && <span className="text-xs text-slate-400 mb-2 truncate">{user.email}</span>}
              <button 
                onClick={handleLogout}
                className={`flex items-center gap-3 p-3 rounded-xl text-slate-600 hover:bg-red-50 hover:text-red-600 transition-all w-full ${!isSidebarOpen && 'justify-center'}`}
              >
            ...
            <Link 
              to="/auth"
              className={`flex items-center gap-3 p-3 rounded-xl text-orange-600 bg-orange-50 hover:bg-orange-100 transition-all w-full font-semibold ${!isSidebarOpen && 'justify-center'}`}
            >
